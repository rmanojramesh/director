(function () {
  ...
  function onHeadingChange(event) {
    ...
    if (typeof heading !== "undefined" && heading !== null) { // && typeof orientation !== "undefined") {
      ...
      var plat =  primlat*Math.PI/180;
      var plong = primlong*Math.PI/180;
      var positionlat = Math.PI*(positionCurrent.lat)/180;
      var positionlng = Math.PI*(positionCurrent.lng)/180;
      var diff_lat=(Math.PI*(positionCurrent.lng - plong))/180;
      var X =  Math.cos(positonlat) * Math.sin(diff_lat);
      var Y = Math.sin(positionlat)*Math.cos(plat) - Math.sin(positionlat)*Math.cos(plat)*Math.cos(diff_lat);
      var bearing = 180*(Math.atan2(X,Y))/Math.PI;


      positionCurrent.hng = bearing + adjustment;
